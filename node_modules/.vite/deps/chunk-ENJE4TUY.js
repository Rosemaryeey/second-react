...
var HSFileUpload = class _HSFileUpload extends HSBasePlugin {
  ...
  previewAccepted(file) {
    ...
    if (remove)
      remove.onclick = () => this.dropzone.removeFile(file);
  }
  onRemoveFile() {
    const trigger = this.el.querySelector(
      "[data-hs-file-upload-trigger]"
  ...
  onUploadProgress(file, progress) {
    ...
    if (progressBarPane)
      progressBarPane.style.width = `${currentProgress}%`;
    if (progressBarValue)
      progressBarValue.innerText = `${currentProgress}`;
  }
  onComplete(file) {
    const { previewElement } = file;
    if (!previewElement)
...
var file_upload_default = HSFileUpload;
...
preline/src/plugins/file-upload/index.ts:
   ...
   *)
*/
//# sourceMappingURL=chunk-ENJE4TUY.js.map
